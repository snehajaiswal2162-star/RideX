...
import { CarTaxiFront, Bike, Car, Bus, Truck, ChevronRight, ChevronLeft, Sparkles } from 'lucide-react';
        ...
        >
          <div>
            <div className='flex items-center gap-2 mb-3'>
              ...
              <span className='text-[10px] font-black text-zinc-400 tracking-[0.2em] uppercase'>Fleet</span>
            </div>

            <h2 className='text-3xl sm:text-4xl font-black leading-none tracking-tight text-zinc-900'>
              Vehicles <br />
              ...
              <span className='relative inline-block'>Categories
                <motion.div
                  ...
                  className='absolute -botton-1 right-0 left-0 h-0.5 bg-zinc-900 origin-left'
                />
              </span>
            </h2>
          ...
          <div className='hidden sm:flex items-center gap-2'>
            <motion.div
              whileTap={{ scale: 0.88 }}
              className='w-11 h-11 rounded-2xl border border-zinc-200 flex items-center justify-center hover:bg-zinc-900 hover:border-zinc-900 hover:text-white disabled:opacity-25 disabled:hover:bg-white disabled:hover:text-zinc-900 disabled:hover:border-zinc-200 transition-all text-zinc-700 shadow-sm'
              onClick = {() => scroll("left")}
            >
              <ChevronLeft />
            </motion.div>
            ...
            {VEHICLE_CATEGORIES.map((c, i) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -8 }}
                  className='group relative min-w-50 sm:min-w-65 shrink-0 cursor-pointer '
                >
                  <motion.div
                    animate={{
                      backgroundColor: isHovered ? "#09090b" : "#ffffff",
                      borderColor: isHovered ? "#09090b" : "#e4e4e7",
                      boxShadow: isHovered ? "0px 24px 56px rgba(0,0,0,0.2)" : "0px 2px 16px rgba(0,0,0,0.06)"
                    }}
                    transition={{ duration: 0.5 }}
                    className='relative rounded-3xl border px-6 sm:p-7 overflow-hidden h-full'
                  >
                    <motion.div
                      animate={{
                    ...
                    <motion.p
                    animate={{color: isHovered ? "rgba(255,255,255,0.8)" : "#a1a1aa"}}
                    transition={{duration:0.2}}
...
>
{
  [
    ...
    {num: "15+", label: "Vehicle types"},
    {num: "24/7", label: "Availability"},
  ].map((d,i)=>{
    return (
    <div className="flex items-center gap-3" key={i}>
      <p className="text-zinc-900 font-black tracking-tight text-lg">{d.num}</p>
      <p className="text-zinc-400 font-medium text-xs">{d.label}</p>
    </div>
    )
  })
}
